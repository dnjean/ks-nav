...
```bash 
    ...
    cd ./container/demo
    # delete the ksbuild folder if it exists and you want 
    # to start with a new environment
    rm -rf ksbuild
    # Build Linux image with debug symbols enabled in a container
    # We will run a manual build - the Dockerfile_linux_app includes 
    podman build -t linux-img -f ./Dockerfile_linux_app .
    podman run -it --name linux-cont linux-img sh
    # cd into linux source code folder within container
    cd /app
    # Configure Linux kernel to enable debug symbols
    make defconfig
    ...
    make oldconfig
    # Build image with debug symbols enabled
    make vmlinux
    # exit container and map container /app folder to a local folder
    exit
    podman cp linux-cont:/app $(pwd)/ksbuild/app
```

At the end of this step, the **./demo/ksbuild/app** folder should be populated with the Linux source and build artifacts. 
...
```bash
    ...
    cp demo/ksnav_kdb.json ./ksbuild/app/.
    cp demo/ksnav_nav.json ./ksbuild/app/.
```
...
First, configure the Dockerfile to run without the webserver 
Update the end of the Dockerfile as follows:
...
```bash
    ...
    # Build the ksnav container 
    podman build -t ksnav-img -f Dockerfile .
    # Run it in interractive mode, mapping the linux image and the output flder
    ...
    exit 
    # Build the database for ks-nav
    cd /app
    nav-db-filler ksnav_kdb.json    
    ...
    nav -f ksnav_nav.json > /out_img/run_init_process
```
Folder **demo/ksbuild/out_img** is now populated with a JPEG image *run_init_process*.

#### 2.2.2 Run ks-nav from Web User Interface
We will re-run the same actions from the CLI section, from the web user interface. 
Configure the Dockerfile to run with the webserver as follows: 
...
ENTRYPOINT ["/usr/bin/start.sh"]
```
Save the Dockerfile and rebuild the ks-nav container
...
```bash
    ...
    nav -f ksnav_nav.json > /out_img/run_init_process
```
Open a web browser at: localhost:8080 and click on `Acquire`
...
```bash
    ...
    instance = 1
    display mode = Functional
    depth = 2
    Click on 'Generate Image'
```
At the end of this step, an image is generated by the tool through the web interface
